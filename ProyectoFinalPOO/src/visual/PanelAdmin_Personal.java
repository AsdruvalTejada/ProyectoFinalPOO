...
public class PanelAdmin_Personal extends JPanel {
	...
	public PanelAdmin_Personal(Color colorAccent) {
        ...
        String[] headers = {"ID", "Nombre Completo", "Rol", "Cédula/ID", "Detalle"};
    ...
    public void cargarPersonal() {
        ...
        for (Medico m : SistemaGestion.getInstance().getListaMedicos()) {
            model.addRow(new Object[]{ m.getId(), m.getNombreCompleto(), "Médico", m.getId(), m.getEspecialidad() });
        }
        for (Secretaria s : SistemaGestion.getInstance().getListaSecretarias()) {
            model.addRow(new Object[]{ s.getId(), s.getNombreCompleto(), "Secretaria", s.getId(), "N/A" });
        }
    }
    ...
    private void modificarSeleccionado() {
        ...
        if (rol.equalsIgnoreCase("Médico")) {
            ...
            if (med != null) {
                // NECESITAS ACTUALIZAR FormRegMedico PARA ACEPTAR OBJETO EN CONSTRUCTOR
                // FormRegMedico form = new FormRegMedico(med); 
                // form.setVisible(true);
                JOptionPane.showMessageDialog(this, "Recuerda actualizar FormRegMedico para aceptar edición (como hicimos con Secretaria).");
            }
